% Close-loop GPS Spoofing Attack Pipeline
% clear;
...
% Single line
T_waypoints = [0, 400];
Waypoints = [0, 0, 10; 1000, 0, 10]; % Repeat the last waypoint to match the time vector size

% Zig-Zag
% T_waypoints = [0, 100, 200, 300, 400];
% Waypoints = [0, 0, 10;
%              400, -200, 10;
%              800, 200, 10;
%              1200, -200, 10;
...
% % Circle
% N_circle = 1000;  % More points for a smoother trajectory
% T_total = 400;  
% T_waypoints = linspace(0, T_total, N_circle)';  
% % Define the radius of the circle
% R = 200;  
% % Define the center of the circle
% xc = R;  % Start at (0,0), so center is at (R,0)
% yc = 0;  
% zc = 10;  
% theta = linspace(-pi, pi, N_circle)';  % Negative for clockwise motion
% Waypoints = [xc + R * cos(theta), ...  % X-coordinates
%              yc + R * sin(theta), ...  % Y-coordinates (clockwise direction)
%              zc * ones(N_circle, 1)];         % Z-coordinates (constant altitude)

% Create the timeseries object
WaypointSeries = timeseries(Waypoints, T_waypoints);

% Random waypoints
% Generate a random trajectory with 10 waypoints from T=0 to T=400
% T_random = linspace(0, 400, 10);
% random_x = cumsum(randn(10, 1) * 200);
% random_y = cumsum(randn(10, 1) * 200);
% random_z = ones(10, 1) * 10;
% Waypoints = [random_x, random_y, random_z];
...
idx_attack_success = find(distances < target_radius_value, 1);  % Find first occurrence
...
if ~isempty(idx_attack_success)
    ...
    % plot(pos_x_visualize(1:idx_attack_success), pos_y_visualize(1:idx_attack_success), 'r', 'LineWidth', 1.5); % Raw position (Red)
    % plot(pos_x_fusion(1:idx_attack_success), pos_y_fusion(1:idx_attack_success), 'b', 'LineWidth', 1.5); % Fused position (Blue)
...
% Extract alarm data
delta_s = logsout.get('delta_s_pid').Values.Data;
lambda_c = logsout.get('S_out').Values.Data;
lambda_r = logsout.get('r').Values.Data;
...
ylabel('||a_s||');
ylim([0,3])
yline(delta_s_value, '--m', '\lambda_a', 'LineWidth', 1.5);
title('Attack Stealthiness Statistics a_s');
grid on;
...
yline(0.75, '--m', '\lambda_{c}', 'LineWidth', 1.5);
ylim([0,3])
grid on;
if ~isnan(attack_success_time)
    xline(attack_success_time, '--k', 'T_{AS}', 'LineWidth', 1.5, 'LabelVerticalAlignment', 'top');
...
xlabel('Time (s)');
ylabel('Anomaly Score');
yline(2.04, '--m', '\lambda_{r}', 'LineWidth', 1.5);
ylim([0,3])
grid on;
if ~isnan(attack_success_time)
    xline(attack_success_time, '--k', 'T_{AS}', 'LineWidth', 1.5, 'LabelVerticalAlignment', 'top');
