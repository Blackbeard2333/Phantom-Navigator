% Randomly generate waypoints or trajectories
...
spoofing_target_x = 600;
spoofing_target_y = 200;
...
% Circular waypoint
% Define the number of waypoints for a smooth circle
N = 1000;  % More points for a smoother trajectory

% Define the total time duration
T_total = 400;  

% Define the time points (Nx1 column vector)
T_waypoints = linspace(0, T_total, N)';  
...
% Define angles for the circular trajectory (clockwise direction)
theta = linspace(-pi, pi, N)';  % Negative for clockwise motion

% Generate waypoints along the circular path (Nx3 matrix)
Waypoints = [xc + R * cos(theta), ...  % X-coordinates
             yc + R * sin(theta), ...  % Y-coordinates (clockwise direction)
             zc * ones(N, 1)];         % Z-coordinates (constant altitude)
